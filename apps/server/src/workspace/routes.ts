...
export function workspaceRoutes(db: Db): RouterType {
  ...
  router.get("/:id/latex/files", async (req, res) => {
    try {
      ...
      const currentFilePath =
        typeof req.query.currentFilePath === "string" ? req.query.currentFilePath : null;
      const latexFiles = await detectWorkspaceMainTexFile({
        rootPath,
  ...
  router.get("/:id/latex/latest", async (req, res) => {
    try {
      const { rootPath } = await getWorkspaceOrThrow(db, req.params.id);
      const mainFilePath = typeof req.query.mainFilePath === "string" ? req.query.mainFilePath : "";

      if (!mainFilePath.trim()) {
        res.status(400).json({ error: "mainFilePath query is required" });
        return;
      }
      ...
      console.error("Failed to read latest LaTeX output:", error);
      res.status(500).json({ error: "Failed to read latest LaTeX output" });
    }
  });
  ...
  router.post("/:id/files/copy", async (req, res) => {
    try {
      ...
      console.error("Failed to copy file/folder:", error);
      res.status(500).json({
