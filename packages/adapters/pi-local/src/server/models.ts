...
import type { AdapterModel } from "@ownlab/adapter-utils";
import { asString, runChildProcess } from "@ownlab/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.OWNLAB_PI_COMMAND === "string" &&
    process.env.OWNLAB_PI_COMMAND.trim().length > 0
      ? process.env.OWNLAB_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["OWNLAB_", "npm_", "NPM_"] as const;
