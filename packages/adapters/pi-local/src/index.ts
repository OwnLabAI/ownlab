...
Use when:
- You want OwnLab to run Pi (the AI coding agent) locally as the agent runtime
...
- OwnLab requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/ownlab/ and resumed with --session.
