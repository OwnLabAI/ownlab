...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveOwnLabHomeDir(), "instances", resolveOwnLabInstanceId(), "config.json");
}
...
function resolveConnectionString(config: PartialConfig | null): string {
  ...
  return `postgres://ownlab:ownlab@127.0.0.1:${port}/ownlab`;
}

function resolveDefaultBackupDir(): string {
  return path.resolve(resolveOwnLabHomeDir(), "instances", resolveOwnLabInstanceId(), "data", "backups");
}
